...
namespace SqlBulkTools
{
    ...
    public class BulkInsertOrUpdate<T> : ITransaction
    {
        ...
        async Task<int> CommitCountTransactionAsync(string connectionName, SqlCredential credentials, SqlConnection connection)
        {
            int affectedRows = 0;
            if (!_list.Any())
            {
            ...
            using (SqlConnection conn = _helper.GetSqlConnection(connectionName, credentials, connection))
            {
                ...
                using (SqlTransaction transaction = conn.BeginTransaction())
                {
                    ...
                    finally
                    {
                        ...
                        commandCount.CommandTimeout = _sqlTimeout;
                        commandCount.Transaction = transaction;

                        var commCount = _helper.CompareSet(_columns, _sourceAlias, _targetAlias, _identityColumn);
                        affectedRows = commandCount.ExecuteNonQuery();
                        //***************
                        conn.Close();
                    }
                }
